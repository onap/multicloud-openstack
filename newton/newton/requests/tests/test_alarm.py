...
class TestAlarm(unittest.TestCase, AbstractTestResource):
    ...
    def setUp(self):
        ...
        self.MOCK_GET_RESOURCES_RESPONSE = {
            "alarms": [
                {"id": "uuid_1", "name": "alarm_1"},
                {"id": "uuid_2", "name": "alarm_2"}
            ]
        }

        self.MOCK_GET_RESOURCE_RESPONSE_NOT_FOUND = {}

        self.assert_keys = "alarms"

        self.HTTP_not_found = status.HTTP_404_NOT_FOUND

    @mock.patch.object(VimDriverUtils, 'get_session')
    @mock.patch.object(VimDriverUtils, 'get_vim_info')
    def test_get_alarms(self, mock_get_vim_info, mock_get_session):
        ...
        response = self.client.get(
            ...
            "alarms", {}, HTTP_X_AUTH_TOKEN=mock_info.MOCK_TOKEN_ID)

        context = response.json()
        self.assertEquals(status.HTTP_200_OK, response.status_code)
        self.assertIsNotNone(context["alarms"])
        self.assertEqual(self.MOCK_GET_RESOURCES_RESPONSE["alarms"], context["alarms"])
