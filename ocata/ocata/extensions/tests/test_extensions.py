...
class TestExtensions(unittest.TestCase):
    ...
    def test_get_extensions_info(self):
        ...
        response = self.client.get(
            "/api/multicloud-ocata/v0/" + vimid + "/extensions/")
        ...
        self.assertEquals("hpa-caps", json_content["extensions"][0]["alias"])
        self.assertEquals("Multiple network support", json_content["extensions"][0]["description"])
        self.assertEquals("HPACapsQuery", json_content["extensions"][0]["name"])
        self.assertEquals("http://127.0.0.1:80/api/multicloud-ocata/v0/%s/extensions/hpa-caps" % vimid,
