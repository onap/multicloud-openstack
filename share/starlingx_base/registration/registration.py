...
class RegistryHelper(newton_registration.RegistryHelper):
    ...
    def _get_system_info(self, cloud_owner, cloud_region_id,  viminfo, session=None):
        ...
        try:
            ...
            systeminfo = self._get_list_resources(
                ...
                "isystems")
            SystemInfo['software_version'] = systeminfo[0].get("software_version", None) if systeminfo else None
            SystemInfo['system_mode'] = systeminfo[0].get("system_mode", None) if systeminfo else None
            SystemInfo['system_type'] = systeminfo[0].get("system_type", None) if systeminfo else None
            return SystemInfo
        except HttpError as e:
            self._logger.error("HttpError: status:%s, response:%s"
                               % (e.http_status, e.response.json()))
    ...
    def _update_k8s_info(self, cloud_owner, cloud_region_id,
        ...
        try:
            ...
            systemversion = systeminfo.get("software_version", None)
