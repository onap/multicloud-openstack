...
def buildBacklog_pm_vm(vimid, backlog_input):
    ...
    try:
        ...
        if server_id is None and server_name is None:
            ...
            api_url_fmt = "/{f_vim_id}/metering/v2/samples?q.field=resource_id&q.op=eq&q.value={f_server_id}"
            api_url = api_url_fmt.format(
                f_vim_id=vimid,
                f_server_id=server_id)
        else:
            api_url_fmt = "/{f_vim_id}/metering/v2/samples"
            api_url = api_url_fmt.format(
                f_vim_id=vimid)
